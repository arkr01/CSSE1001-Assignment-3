...
class GridCrafterView(tk.Frame):
    ...
    def __init__(self, master, input_size):
        ...
        # ...
        self._input_window = ItemGridView(self, input_size)
        self._input_window.pack(side=tk.LEFT)

        self._craft_button = tk.Button(self, text="=> Craft =>")

        # Customise Craft Button for Furnace and adds a fire sprite image
        if input_size == (2, 1):
            self._craft_button.config(text="=> Smelt =>")

            self._fire_label = tk.Label(self)
            self._fire_label.pack(side=tk.LEFT)

            self._fire_image = tk.PhotoImage(file="fire_image.gif")
            self._fire_label.configure(image=self._fire_image)

        self._craft_button.pack(side=tk.LEFT)

        self._output_window = ItemGridView(self, (1, 1))
        self._output_window.pack(side=tk.LEFT)
    ...
    def render(self, key_stack_pairs, selected):
        ...
        for key, stack in key_stack_pairs:
            ...
            if key == "output":
                ...
                # ...
                self._output_window.draw_cell((0, 0), stack, key == selected)

            else:
                # Task 2.2 Crafting: Draw input cells
                # ...
                self._input_window.draw_cell(key, stack, key == selected)
    ...
    def bind_for_id(self, event, callback):
        ...
        self._input_window.bind(event, lambda e: callback(
            self._input_window.xy_to_grid((e.x, e.y)), e))
        self._output_window.bind(event, lambda e: callback("output", e))
...
class CraftingWindow(tk.Toplevel):
    ...
    def __init__(self, master, title, hot_bar: Grid, inventory: Grid, crafter: GridCrafter):
        ...
        for widget_key in ('inventory', 'hot_bar'):
            ...
            view_widget.bind_for_id("<Button-3>",
                                    lambda key, e,
                                           widget_key=widget_key: self._handle_right_click(
                                        widget_key, key, e))
        self.redraw()
    ...
    def _load_crafter_view(self):
        ...
        crafter_view.bind_for_id("<Button-2>", lambda key, e: self._handle_right_click("crafter", key, e))  # For MacOS Users
        crafter_view.bind_for_id("<Button-3>", lambda key, e: self._handle_right_click("crafter", key, e))  # For Windows Users
