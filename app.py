...
__author__ = "Adrian Rahul Kamal Rajkamal"
__date__ = "31/05/2019"
__version__ = "1.1.0"
...
def create_block(*block_id):
    ...
    # To allow for smelting and placing diamonds blocks!
    BREAK_TABLES_WITH_DIAMOND = {
        "diamond": {
            "hand": (7.5, False),
    ...
    if len(block_id) == 1:
        ...
        if block_id == "leaf":
            ...
            return ResourceBlock(block_id, BREAK_TABLES_WITH_DIAMOND[block_id])
...
def create_item(*item_id):
    ...
    CRAFTING_AND_SMELTING_ITEMS = ("stick", "fence", "coal", "bucket", "paper",
                                   "map", "torch", "cobblestone", "brick",
                                   "soot")

    if len(item_id) == 2:
        if item_id[0] in MATERIAL_TOOL_TYPES and item_id[1] \
                in TOOL_DURABILITIES:

            # Returns all tools in general form:
            # "{material}_{tool_type}", "TOOL_TYPE", "TOOL_DURABILITY[material]"

            return ToolItem(f"{item_id[1]}_{item_id[0]}", item_id[0],
    ...
    elif item_type == "poison_apple":
        return FoodItem(item_type, -2)

    # Task 1.4 Basic Items: Create wood & stone here
    # ...

    elif item_type in ("wood", "stone", "diamond"):
        return BlockItem(item_type)
    elif item_type in CRAFTING_AND_SMELTING_ITEMS:
        return SimpleItem(item_type)
    elif item_type in ("crafting_table", "furnace"):
        return BlockItem(item_type)
...
class FoodItem(Item):
    """Items (i.e. food) that increase player's food or health."""

    def __init__(self, id_, strength):
        """Constructor

        Parameters:
            id_ (str): The unique ID of the food
            strength (float): The amount the food increases the player's
            health/food
        """
    ...
    def get_strength(self):
        """(float) Returns the amount the food increases the player's
            health/food."""
        return self._strength

    def place(self):
        """(float) Returns an effect that represents an increase in the
        player's food/health."""
        return [('effect', ('food', self.get_strength()))]

    def can_attack(self):
        """(bool) Returns True iff food can attack, which it obviously can't."""
        return False
    ...
    def attack(self, successful):
        ...
        Return:
            [tuple<str, tuple<str, ...>>]:
                    A list of EffectIDs resulting from the attack.
                    Each EffectID is a pair of (effect_type, effect_sub_id)
                    pair, where:
                      - effect_type is the type of the effect
                      ('item', 'block', etc.)
                      - effect_sub_id is the unique identifier for an effect of
                       a particular type
        """
        pass

    def get_durability(self):
        """(float) Returns the item's durability (effectively its health).
        For items that cannot attack, this value is irrelevant"""
        pass
...
class ToolItem(Item):
    """Items (i.e. tools) that allow for more efficient mining."""

    def __init__(self, id_, tool_type, durability):
        """Constructor

        Parameters:
            id_ (str): The unique ID of the tool
            tool_type (str): The type of tool (e.g. axe, sword, etc.)
            durability (float): The durability of the tool
            (i.e. how much mining it can complete before being depleted)
        """
    ...
    def get_type(self):
        """(str) Returns the type of tool (i.e. axe, sword, etc.)."""
        return self._tool_type

    def get_durability(self):
        """(float) Returns the current durability of the tool."""
        return self._durability

    def get_max_durability(self):
        """(float) Returns the initial (or maximum) durability of the tool."""

        # Get's the tool ID, which is in the form '{material}_{tool_type}',
        # splits the underscore to create a list of those two elements
        # (material and tool_type), and then accesses the durability of the
        # material
        return TOOL_DURABILITIES[self.get_id().split('_')[0]]

    def can_attack(self):
        """(bool) Returns True iff the tool can still attack, i.e. if the tool
        isn't depleted yet."""
        return self._durability > 0

    def attack(self, successful):
        """Attacks/mines with the tool. Checks if the attack wasn't
         successful and reduces durability as a result.

         Parameters:
             successful (bool): Checks whether or not the attack was successful.
         """
    ...
    def place(self):
        ...
        Return:
            [tuple<str, tuple<str, ...>>]:
                    A list of EffectIDs resulting from placing this item.
                    Each EffectID is a pair of (effect_type, effect_sub_id)
                    pair, where:
                      - effect_type is the type of the effect
                      ('item', 'block', etc.)
...
class CraftingTableBlock(ResourceBlock):
    """A block that allows for more sophisticated crafting."""

    def __init__(self):
        """Constructor"""
        super().__init__("crafting_table", BREAK_TABLES['wood'])
    ...
    def get_drops(self, luck, correct_item_used):
        """Drops the Crafting Table in item form.

        Parameters:
            luck (float): The player's current luck factor, a random number
            between [0, 1)
            correct_item_used (bool): Checks if the item used to
            mine the crafting block was the correct form to
            get the crafting table in drop form.
        ...
        Return:
            list<
                tuple<
                    ...
                    tuple<str, ...>
                >
            >: A list of effects dropped by this block.
            Specifically, the crafting table item.
        """
        if correct_item_used:
            return [('item', ('crafting_table',))]

    def can_use(self):
        """(bool) Returns True iff the crafting table can be used."""
        return True
    ...
    def use(self):
        ...
        Return:
            <tuple>: Returns the effect of using the crafting table."""

        return ('crafting', 'crafting_table')


class FurnaceBlock(ResourceBlock):
    """A block that allows for smelting."""

    def __init__(self):
        """Constructor"""
        super().__init__("furnace", BREAK_TABLES['stone'])
    ...
    def get_drops(self, luck, correct_item_used):
        ...
        Parameters:
            luck (float): The player's current luck factor, a random number
            between [0, 1)
            correct_item_used (bool): Checks if the item used to mine the
            furnace was the correct form to get the furnace in drop form.
...
FURNACE_RECIPES = {
    ...
    (
        ...
        Stack(create_item('cobblestone'), 4)
    ),
    (
        (
            ('cobblestone',),
    ...
    (
        ...
        Stack(create_item('poison_apple'), 2)
    ),
    (
        (
            ('coal',),
...
class StatusView(tk.Frame):
    """Frame Widget to display the player's current health and food."""

    def __init__(self, master):
        """Constructor

        Parameters:
            master (tk.Tk): The game window
        """
        super().__init__(master)

        self._health_label = tk.Label(self)
        self._health_label.pack(side=tk.LEFT)

        self._heart_image = tk.PhotoImage(file="heart_image.gif")
        self._health_label.configure(image=self._heart_image)

        self._health_label = tk.Label(self, text="Health: 20.0")
        self._health_label.pack(side=tk.LEFT)

        self._food_label = tk.Label(self)
        self._food_label.pack(side=tk.LEFT)

        self._drumstick_image = tk.PhotoImage(file="drumstick_image.gif")
        self._food_label.configure(image=self._drumstick_image)

        self._food_label = tk.Label(self, text="Food: 20.0")
        self._food_label.pack(side=tk.LEFT)

    def set_health(self, health):
        """Updates the player's health as the game
        progresses (in 0.5 intervals).

        Parameters:
            health (float): The player's current health
        """
        self._health_label.config(text=f"Health: {str(round(health)/2)}")
    ...
    def set_food(self, food):
        ...
        Parameters:
            food (float): The player's current health
        """
        self._food_label.config(text=f"Food: {str(round(food)/2)}")
...
class Ninedraft:
    ...
    def __init__(self, master):
        ...
        # Loads the world, player, inventory and hotbar
        self._new_game()

        self._view = GameView(self._master, self._world.get_pixel_size(),
        ...
        self._view.bind("<Button-2>", self._right_click)  # For MacOS Users
        self._view.bind("<Button-3>", self._right_click)  # For Windows Users
        self._view.bind("<Enter>", self._mouse_move)
        ...
        for i in range(1, 10):
            self._master.bind(str(i),
                              lambda e, j=i - 1: self._hot_bar.toggle_selection(
                                  (0, j)))  # j=i-1 due to zero-based indexing
        ...
        # No need for detachable menus, hence 0 tear-off
        self._file_menu = tk.Menu(self._menu_bar, tearoff=0)
        self._menu_bar.add_cascade(label="File", menu=self._file_menu)
        self._file_menu.add_command(label='New Game', command=self._restart)
        self._file_menu.add_command(label='Exit', command=self._close)

        # Allows Alt+F4 (and MacOS equivalent) and the red X button to also
        # close the game
    ...
    def _new_game(self):
        ...
        self._world.add_collision_handler("player", "item",
                                          on_begin=self._handle_player_collide_item)
        ...
        self._master.bind("e", lambda e: self.run_effect(('crafting', 'basic')))

    def _close(self):
        """Closes the game window if the user confirms."""

        exit_question = messagebox.askquestion("Exit",
    ...
    def _restart(self):
        """Sets up a new game if the player confirms."""

        new_game_question = messagebox.askquestion(
            "New Game", "Are you sure you would like to start a new game?")

        if new_game_question == "yes":
            self._new_game()
    ...
    def step(self):
        ...
        if self._player.get_health() == 0:
            restart_question = messagebox.askquestion(
                "You Died!", "You died! Would you like to restart?")
    ...
    def _jump(self):
        ...
        # ...
        self._player.set_velocity((velocity.x * 2,
                                   velocity.y + -200))
    ...
    def mine_block(self, block, x, y):
        ...
        if block.is_mined():
            ...
            if self._player.get_food() > 0:
                self._player.change_food(-0.7)
            else:
                self._player.change_health(-0.7)
    ...
    def _trigger_crafting(self, craft_type):
        ...
        # Creates different crafting windows and binds the 'e' key to close the
        # basic crafting window

        if craft_type == 'basic':
            basic_crafter = GridCrafter(CRAFTING_RECIPES_2x2)
            self._basic_window = CraftingWindow(self._master, "Crafting",
                                                self._hot_bar,
                                                self._inventory, basic_crafter)
            self._basic_window.bind("e", lambda e: self._basic_window.destroy())

        elif craft_type == 'crafting_table':
            table_crafter = GridCrafter(CRAFTING_RECIPES_3x3, 3, 3)
            CraftingWindow(self._master, "Crafting Table", self._hot_bar,
                           self._inventory, table_crafter)

        elif craft_type == 'furnace':
            furnace_crafter = GridCrafter(FURNACE_RECIPES, 2, 1)
            CraftingWindow(self._master, "Furnace", self._hot_bar,
                           self._inventory, furnace_crafter)
        ...
        if len(effect) == 2:
            if effect[0] == "crafting":
                ...
                if craft_type == "basic":
                    ...
                    print("Light 'em up up up I'm on fiyaahhhhhh! "
